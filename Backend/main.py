...
# en static/overlays (configurable). Soporta /predict (1 imagen) y /predict_batch (varias).
...
def save_visuals_and_polys(img_gray: np.ndarray, heat_norm: np.ndarray,
    ...
    Aplica ROI para máscara/polígonos. Guarda en static/OVERLAYS_SUBDIR y devuelve:
      - rutas (overlay/heat/mask)
      - polígonos
      - áreas_px por polígono
      - URLs públicas (overlay/heat/mask)
    """
...
async def predict_batch(
    ...
    n_anom = 0
    n_norm = 0
    all_defect_areas = []  # todas las áreas (px) de todos los polígonos de todas las imágenes
    ...
    for f in files:
        ...
        # 3) umbral (mismo para todas en este batch)
        threshold = threshold_base
        is_anomaly = bool(score > threshold)

        # 4) visual + polígonos + áreas
        overlay_url = None
        polygons: List[List[List[int]]] = []
    ...
    summary = {
        ...
        "anomalies": n_anom,
        "normals": n_norm,
        "defect_rate": defect_rate,             # 0..1
        "avg_defect_area_px": avg_defect_area   # pixels en imagen IMG_SIZE x IMG_SIZE
    }
